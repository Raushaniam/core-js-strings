...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    return str.slice(0, index1) + str.slice(index1 + value.length, str.length);
  }
  return str;
}
...
function reverseString(str) {
  return String(str.split('').reverse()).replaceAll(',', '');
}
...
function orderAlphabetically(str) {
  return String(str.split('').sort()).replaceAll(',', '');
}
...
function isPalindrome(str) {
  ...
  return String(string.split('').reverse()).replaceAll(',', '') === string;
}
...
function encodeToRot13(str) {
  ...
  arr.forEach((item) => {
    ...
    resultString += String.fromCharCode(count);
  });
  return resultString;
}
